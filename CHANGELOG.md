...
- **`HapbeatAddressOverridePanel`（Runtime コンポーネント）** — GameObject に 1 つ追加するだけで override 設定 UI が出ます。`ScreenSpaceOverlay` / `WorldSpace`（VR 用）の 2 モードに対応。Player -/+、Group -/+、Play、Apply、Exit のボタンを内蔵し、2D フォーカスグリッド（`RegisterFocusable` / `MoveFocus` / `ActivateFocused`）でコントローラー操作にも対応します。Play / Exit の実処理は `OnPlayRequested` / `OnExitRequested` で外部から注入します。
  ...
  - `WorldSpace` 時の **composition layer モード（opt-in）**（`World Attach Mode` = `CompositionLayer`）— パネルを RenderTexture に描画し、OpenXR の **quad composition layer** として XR コンポジタに直接渡します。アプリのアイバッファを経由しないため、(a) 再投影（TimeWarp）の後段で合成されるので視界に固定しても泳がず、(b) Render Scale の影響を受けないため文字が鮮明になります（Quest の起動ロゴと同じ仕組み）。**利用側プロジェクトに `com.unity.xr.compositionlayers` を導入し、`Project Settings > XR Plug-in Management > OpenXR` の **Composition Layers** feature を有効にする必要があります**。パッケージ未導入・feature 無効・レイヤープロバイダ未起動のいずれの場合も、警告を 1 回出して `LazyFollow` にフォールバックします（SDK 自体はパッケージ無しでもコンパイルできます。asmdef の `versionDefines` による opt-in で、`package.json` に依存は追加していません）。既定は従来どおり `LazyFollow` です。
