...
- **`HapbeatAddressOverridePanel`（Runtime コンポーネント）** — GameObject に 1 つ追加するだけで override 設定 UI が出ます。`ScreenSpaceOverlay` / `WorldSpace`（VR 用）の 2 モードに対応。Player -/+、Group -/+、Play、Apply、Exit のボタンを内蔵し、2D フォーカスグリッド（`RegisterFocusable` / `MoveFocus` / `ActivateFocused`）でコントローラー操作にも対応します。Play / Exit の実処理は `OnPlayRequested` / `OnExitRequested` で外部から注入します。
  ...
  - `WorldSpace` 時の **composition layer モード（opt-in）**（`World Attach Mode` = `CompositionLayer`）— パネルを RenderTexture に描画し、OpenXR の **quad composition layer** として XR コンポジタに直接渡します。アプリのアイバッファを経由しないため、(a) 再投影（TimeWarp）の後段で合成されるので視界に固定しても泳がず、(b) Render Scale の影響を受けないため文字が鮮明になります（Quest の起動ロゴと同じ仕組み）。**利用側プロジェクトで 3 つの設定が必要です**: (a) `com.unity.xr.compositionlayers` の導入、(b) `Project Settings > XR Plug-in Management > OpenXR` の **Composition Layers** feature 有効化、(c) `Hapbeat > Settings` の `XR > Enable Composition Layer Support`（`HapbeatConfig.enableCompositionLayerSupport`、既定 OFF）。いずれかが欠けている場合・レイヤープロバイダ未起動の場合は、警告を 1 回出して `LazyFollow` にフォールバックします（SDK 自体はパッケージ無しでもコンパイルできます。asmdef の `versionDefines` による opt-in で、`package.json` に依存は追加していません）。既定は従来どおり `LazyFollow` です。
    - (c) が必要な理由: OpenXR の Composition Layers feature がレイヤープロバイダを割り当てるのは **XR セッション開始時の一度きり**で、しかも**その瞬間に composition layer マネージャが起動している場合のみ**です。XR の初期化・セッション開始（`XRGeneralSettings` の `AfterAssembliesLoaded` / `BeforeSplashScreen` フック）は最初のシーン読み込みより前に走るため、シーン上のコンポーネントから作ったレイヤーでは常に手遅れになります。(c) を有効にすると SDK が `SubsystemRegistration` でマネージャを起動状態にし（`HapbeatCompositionLayerBootstrap`、空・非描画のレイヤーを 1 枚常駐させる）、この割り当てが成立します。常駐コストがあるため既定は OFF です。
    - 制約: このモードでは Canvas をシーン外に退避して撮影するため、パネルへのポインター（マウス / レイ）操作は効きません（フォーカスグリッド操作を使ってください）。また撮影されるのはパネル Canvas の矩形内のみで、`PanelCanvasTransform` の下に矩形外へオフセットして付けた UI は写りません。
...
- **`World Attach Mode = CompositionLayer` が実機で一度も有効にならない** — OpenXR の `Composition Layers` feature を有効にしていても、パネルが常に「レイヤープロバイダが利用できない」と警告して `LazyFollow` にフォールバックしていました。原因は composition layers パッケージ側のライフサイクルで、`OpenXRCompositionLayersFeature` はレイヤープロバイダを **XR セッション開始時の 1 回だけ**、しかも**その瞬間に `CompositionLayerManager` のインスタンスが生きている場合に限り**割り当てます。マネージャはレイヤーが 1 つも無いと自ら停止するため、必要になってから初めてレイヤーを作る構成（このパネルもそれ）では、セッション開始時点でマネージャが停止済み → プロバイダは永久に割り当てられない、という状態になっていました。パネル生成時にテクスチャを持たない保持用レイヤーを 1 つ確保してマネージャを起動状態に保つよう修正しています（保持用レイヤーは swapchain を作らないため描画・メモリへの影響はありません）。
- composition layer モードのフォールバック警告が原因を区別できるようになりました（パッケージ未導入 / マネージャ停止 / プロバイダ未割り当て）。成功時は解像度とレイヤー順を info ログに出します。またプロバイダ待ちの上限を 2 秒 → 10 秒に延長しました（Air Link ではセッション開始まで数秒かかります）。
- composition layer モード中に XR セッションが終了しても（ヘッドセットを外す等）パネルが消えないようにしました。プロバイダが失われている間はアイバッファ描画に自動で戻り、セッション再開で composition layer に復帰します。
- composition layer モードで文字が左右反転して撮影される問題を修正しました（撮影カメラが Canvas の裏側から撮っていたため）。
