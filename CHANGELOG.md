...
- **CLIP (Wi-Fi UDP stream) が `streamBufferMs` 有効化後に以前より途切れるようになっていた退行を修正（2 点）**
  - **チャンクサイズ**: `MixerCoroutine` は `STREAM_DATA_MAX_PAYLOAD`（1400 バイト）を丸ごと使い切る粒度で 1 パケットを送っていた（mono 700 frames = 43.75 ms、stereo 350 frames = 21.875 ms @16 kHz）。device 側の低遅延リング (`audio_stream.cpp`) は `streamBufferMs`（既定 30）で target=30 ms/hard-trim ceiling=45 ms に設定されるため、mono パケット 1 個の到着だけでリング fill が ceiling を超え、次の mix tick で fill が target まで即座に hard-trim（`audioStreamMixInto` の burst-overrun 分岐）— 新着チャンクのほぼ全部が毎パケット捨てられていた。1 パケットあたりの音声長を ~10 ms に縮小し、target/ceiling に対して十分な余裕を残すよう修正（`HapbeatProtocol.STREAM_DATA_MAX_PAYLOAD` によるキャップは維持）。
  - **送信ペーシングの lead クランプ**: チャンクサイズを縮めても、送信ペーシングの先行量 `streamSendAheadSeconds`（既定 0.05 = 50 ms）自体が `streamBufferMs` の target（既定 30 ms → ceiling 45 ms）を上回っていたため、定常状態の fill が target を常に超過し続け、hard-trim が毎 tick 発生する退行が残っていた。`MixerCoroutine` のペーシングで実効 lead を `streamBufferMs > 0` の場合 `min(streamSendAheadSeconds, streamBufferMs / 1000)` にクランプし、fill が target 以下に収まるよう修正（`streamBufferMs = 0`、device 既定の低遅延モードでは従来どおり無制限）。
